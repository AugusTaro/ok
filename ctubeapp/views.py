...
def player_view(request):
    ...
    return render(request, 'demo/player.html',context )  #player.htmlにURLパラメータから受け取った値を渡す
...
#非検索状態の処理。
        ...
        video_infomation_category1 = video.objects.filter(group_ID=group_ID, category_ID=1).values()
        video_infomation_category2 = video.objects.filter(group_ID=group_ID, category_ID=2).values()
        video_infomation_category3 = video.objects.filter(group_ID=group_ID, category_ID=3).values()
        video_infomation_category4 = video.objects.filter(group_ID=group_ID, category_ID=4).values()
